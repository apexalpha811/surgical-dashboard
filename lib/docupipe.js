(function(){
  ...
  async function api(path, options) {
    ...
    if (!response.ok) {
      const error = new Error(`${method} ${path} failed with ${response.status}${payload.error ? `: ${payload.error}` : ""}${payload.message ? `: ${payload.message}` : ""}`);
  ...
  async function standardizeDocument() {
    ...
    const result = await api("/api/docupipe/standardize", {
      method: "POST",
      body: JSON.stringify({
        moduleId: module.id,
        schemaId: module.docupipeSchemaId,
