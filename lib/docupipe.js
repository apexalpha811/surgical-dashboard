(function(){
  ...
  const SCHEMA_OVERRIDES_KEY = "docupipe-schema-overrides-v1";
  const MAX_UPLOAD_BYTES = 60 * 1024 * 1024;
  const MAX_UPLOAD_LABEL = "60 MB";
  ...
  function targetForDocumentType(documentType) {
    switch (String(documentType || "").toLowerCase()) {
      case "claim": return "professionalClaim837P";
      case "eligibility": return "eligibility270";
      case "era": case "denial": case "eob": return "appealsFromEra";
      case "attachment": return "claimAttachment275";
      case "providerenrollment": return "providerEnrollment";
      default: return null;
    }
  }
  ...
  function saveSchemaOverride(moduleId, schemaId) {
    if (!moduleId || !schemaId) return;
    try {
      const overrides = loadSchemaOverrides();
      overrides[moduleId] = schemaId;
      localStorage.setItem(SCHEMA_OVERRIDES_KEY, JSON.stringify(overrides));
    } catch {
      // The server save still runs. This fallback only protects browser refreshes on ephemeral hosts.
    }
  }

  function applySchemaOverrides(modules) {
    const overrides = loadSchemaOverrides();
    return modules.map(module => overrides[module.id] ? { ...module, docupipeSchemaId: overrides[module.id] } : module);
  }
  ...
  async function loadModules() {
    ...
    state.modules = applySchemaOverrides(result.modules || []);
    const claimModule = state.modules.find(module => module.id === "claim-intake-837p");
    state.selectedModuleId = state.selectedModuleId || (claimModule && claimModule.id) || (state.modules[0] && state.modules[0].id) || "";
    renderModuleSelect();
    populateEditor();
  ...
  function populateEditor() {
    ...
    renderSchemaSelect(module.docupipeSchemaId || "");
  ...
  function schemaLabel(schema) {
    return `${schema.schemaName || "Untitled schema"} (${schema.schemaId})`;
  }

  function renderSchemaSelect(selectedSchemaId) {
    const select = $("docuSchemaSelect");
    if (!select) return;
    if (!state.availableSchemas.length) {
      select.innerHTML = `<option value="">Refresh schemas to load</option>`;
      select.value = "";
      return;
    }
    select.innerHTML = `<option value="">Choose a DocuPipe schema</option>${state.availableSchemas.map(schema => (
      `<option value="${escapeAttr(schema.schemaId)}">${escapeHtml(schemaLabel(schema))}</option>`
    )).join("")}`;
    select.value = selectedSchemaId && state.availableSchemas.some(schema => schema.schemaId === selectedSchemaId) ? selectedSchemaId : "";
  }

  async function refreshSchemas() {
    setText("docuSchemaState", "loading schemas");
    const result = await api("/api/docupipe/schemas?limit=1000&exclude_payload=true");
    state.availableSchemas = Array.isArray(result.schemas) ? result.schemas : [];
    renderSchemaSelect($("docuSchemaId") ? $("docuSchemaId").value.trim() : "");
    setText("docuSchemaState", state.availableSchemas.length ? `${state.availableSchemas.length} schemas` : "no schemas found");
    toast(state.availableSchemas.length ? `Loaded ${state.availableSchemas.length} DocuPipe schemas.` : "No DocuPipe schemas returned.");
    return result;
  }
  ...
  function chooseSchema() {
    ...
    const schema = state.availableSchemas.find(item => item.schemaId === select.value);
    if ($("docuSchemaId")) $("docuSchemaId").value = select.value;
    setText("docuSchemaState", `selected: ${select.value}`);
    toast(schema ? `Selected ${schemaLabel(schema)}.` : `Selected schema ${select.value}.`);
  }

  async function saveSchemaId() {
    const current = selectedModule();
    if (!current || !current.id) {
      toast("Choose a module first.");
      return null;
    }
    const schemaId = $("docuSchemaId") ? $("docuSchemaId").value.trim() : "";
    if (!schemaId) {
      toast("Enter a DocuPipe schema ID first.");
      return null;
    }
    saveSchemaOverride(current.id, schemaId);
    const module = { ...collectModule(false), docupipeSchemaId: schemaId };
    let result;
    try {
      result = await api(`/api/modules/${encodeURIComponent(current.id)}`, {
        method: "PUT",
        body: JSON.stringify(module)
      });
    } catch (error) {
    ...
    state.modules = state.modules.map(item => item.id === current.id ? { ...result.module, docupipeSchemaId: schemaId } : item);
    state.selectedModuleId = current.id;
    renderModuleSelect();
    populateEditor();
    setText("docuSchemaState", `saved: ${schemaId}`);
    toast(`Saved DocuPipe schema ${schemaId}.`);
    return result;
  }
  ...
  async function validateSchema() {
    ...
    setText("docuSchemaState", "checking");
    const result = await api(`/api/docupipe/schemas/${encodeURIComponent(schemaId)}`);
    setText("docuSchemaState", `valid: ${schemaId}`);
    toast(`DocuPipe schema ${schemaId} is accessible.`);
    return result;
  }
  ...
  async function runIntake() {
    ...
    if (!file) {
      toast("Choose a source file to run intake.");
      return;
    }
    // Clear any prior run so a freshly chosen file re-runs the whole chain cleanly.
    state.upload = null;
    state.standardize = null;
    state.standardization = null;
    state.preview = null;
    state.importedPreviewKey = "";
    try {
      // standardizeDocument uploads if needed, then chains to preview + auto-import.
      await standardizeDocument();
    } catch (error) {
      // standardizeDocument already surfaces credit/errors via toast; keep the UI responsive.
      console.warn(error);
    }
  }
  ...
  async function uploadDocument() {
    ...
    if (file.size > MAX_UPLOAD_BYTES) {
      ...
      toast(`File is too large for Cloudflare upload. Choose a file under ${MAX_UPLOAD_LABEL}.`);
      setText("docuDocumentId", `file over ${MAX_UPLOAD_LABEL}`);
      return null;
    }
  ...
  async function autoImportPreview(preview) {
    ...
    // If DocuPipe reports a document type that differs from what we standardized against,
    // only auto-file it when a configured schema exists for that type. Otherwise warn
    // instead of silently routing the data into the wrong dashboard section.
    const detectedType = preview.standardization && preview.standardization.documentType;
    const detectedTarget = targetForDocumentType(detectedType);
    if (detectedTarget && detectedTarget !== preview.target) {
      const detectedModule = state.modules.find(module => module.stediTarget === detectedTarget);
      if (!detectedModule || !detectedModule.docupipeSchemaId) {
        toast(`Detected a ${detectedType} document, but no schema is configured for that type yet, so it was not auto-filed.`);
        return;
      }
    }
  ...
  async function commitPreviewToDashboard(preview, options) {
    ...
    saveImportedHistory();
    persistImport(preview, record).catch(error => console.warn(error));
    renderImportedState();
    toast(auto
  ...
  async function persistImport(preview, record) {
    const module = activeModuleDraft();
    const standardization = state.standardization || {};
    const upload = state.upload || {};
    await api("/api/imports", {
      method: "POST",
      body: JSON.stringify({
        moduleId: module && module.id,
        documentId: upload.documentId || standardization.documentId,
        standardizationId: standardization.standardizationId,
        target: preview.target,
        status: preview.warnings && preview.warnings.length ? "partial" : "imported",
        extracted: preview.standardization || standardization,
        standardization,
        preview,
        record,
  ...
  function init() {
    ...
    bind("docuFile", "change", e => {
      ...
      if (state.selectedFile && state.selectedFile.size > MAX_UPLOAD_BYTES) {
        toast(`File is too large for Cloudflare upload. Choose a file under ${MAX_UPLOAD_LABEL}.`);
        state.selectedFile = null;
        e.target.value = "";
        setText("docuDocumentId", `file over ${MAX_UPLOAD_LABEL}`);
      ...
      if (state.selectedFile) {
        toast(`Selected ${state.selectedFile.name}. Running automated intake...`);
        runIntake();
      }
    });
    bind("docuRunBtn", "click", runIntake);
    ...
    bind("docuRefreshSchemasBtn", "click", refreshSchemas);
    bind("docuSchemaSelect", "change", chooseSchema);
    bind("docuSaveSchemaBtn", "click", saveSchemaId);
    bind("docuValidateSchemaBtn", "click", validateSchema);
